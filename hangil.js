  ...
  var HanTools = {
    ...
    toChoseong: function (s) {
      ...
      if (s.length == 1) {
        ...
        if (c >= HANGUL_FIRST_CODE && c <= HANGUL_LAST_CODE)
          return CHOSEONG[
            0 | ((c - HANGUL_FIRST_CODE) / (JUNGSEONG_LEN * JONGSEONG_LEN))
          ];
        else return s;
      }
      r = '';
    ...
    toJungseong: function (s) {
      ...
      if (s.length == 1) {
        ...
        if (c >= HANGUL_FIRST_CODE && c <= HANGUL_LAST_CODE)
          return JUNGSEONG[
            0 | ((c - HANGUL_FIRST_CODE) / JONGSEONG_LEN) % JUNGSEONG_LEN
          ];
        else return s;
      }
      r = '';
    ...
    compose: function (a) {
      ...
      return String.fromCharCode(
        HANGUL_FIRST_CODE + z + JONGSEONG_LEN * (y + JUNGSEONG_LEN * x)
      );
    },
    dueum: function (s) {
      if (!s) return '';
    ...
    readNumber: function (n) {
      ...
      for (; (n = Math.floor(n / 10000)); i++) {
        if (n % 10000)
          a.unshift(readNumber_thousands(n % 10000) + '만억조경'[i]);
      }
      return a.join(' ');
    },
    noConflict: function () {
